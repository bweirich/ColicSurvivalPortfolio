...
## Purpose 
Colic in horse is problem with the gastro-intestinal tract that causes abdominal pain. Colic can vary between mild cases to sever cases which can involve surgery. The cost of treatment can also vary greatly with the severity of the case.  

This analysis will predict the survival of a horse based on the features of the medical condition of a horse. The ability to predict a horse's survival will allow owners and veterinarians to make better decisions regarding the best interest of the horse and the cost to the owner.

## EDA (Exploratory Data Analysis)

The feature variables distribution was examined and the handling of null values was determined.  I created functions to assist with the visualization of numerical features and categorical features as well as functions to assist in the filling in of null values. 
...
There are 3 different lesion feature variables.  As the values associated with the lesion variables are categorical rather than numerical the columns can be combined into one and the count of lesions can be recorded and used instead. 
...
## Select best model  
Based on the Classification Report measurements of f1-scores and overall accuracy the Logistic Regression is the best model for prediction.  

## Conclusion 
Since the training dataset is fairly small, we would recommend that more data is collected for the models to train on before the model is deployed for real world use in predictions.
